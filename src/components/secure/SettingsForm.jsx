    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.data) {
                const singin_response = await api.post(AUTH.SIGNIN, credentials, {
                    timeout: 5000
                });

                if (singin_response.data && singin_response.data.data.token) {
                    localStorage.setItem("token", singin_response.data.data.token)
    ...
    return (
        <Container className="mt-5">
            <Form onSubmit={handleSubmit} className="mx-auto bg-dark text-light p-4 rounded" style={{ maxWidth: '400px' }}>
                <h2 className="mb-4 text-light">Sign Up</h2>
                ...
                <Form.Group className="mb-3">
                    ...
                    <Form.Control className="bg-dark text-light border-secondary" type="text" name="email" value={credentials.email} onChange={handleChange} required disabled={isLoading} />
                </Form.Group>
                <Form.Group className="mb-3">
